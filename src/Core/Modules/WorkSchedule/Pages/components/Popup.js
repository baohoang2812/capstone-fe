...
const Popup = (props) => {
  ...
  const dataChangedShiftHandler = ({ target }) => {
    ...
    const duration = moment.duration(
      moment(newShift?.[0].endTime, "HH:mm:ss").diff(
        moment(newShift?.[0].startTime, "HH:mm:ss")
      )
    );
    ...
    setState({
      ...
      startDate:
        ...
        " " +
        moment(newShift?.[0].startTime, "HH:mm:ss").format("HH:mm"),
      endDate:
        moment(state.endDate).format("YYYY-MM-DD") +
        " " +
        moment(newShift?.[0].endTime, "HH:mm:ss").format("HH:mm"),
      [target.name]: target.value,
    });
  };
  ...
  const shiftItems = shifts.map((resource) => (
    <MenuItem key={resource.id} value={resource.id}>
      {resource.name}({moment(resource.startTime, "HH:mm:ss").format("HH:mm")} -{" "}
      {moment(resource.endTime, "HH:mm:ss").format("HH:mm")})
    </MenuItem>
  ));
  ...
  const handleRemoveSchedule = async () => {
    ...
    const res = await workScheduleApi.delete(body);
    if (res.code === 201 || res.code === 200) {
      message.success(t("CORE.task_success"));
      props.removeRevordHandler();
      props.closePopup();
    } else {
      message.error(t("CORE.task_failure"));
      return;
