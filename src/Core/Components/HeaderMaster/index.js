  ...
  const callApi = () => {
    (async () => {
      ...
      listNoti?.length > 0 && data.forEach(item => {
        ...
        if (is_new) {
          notification["error"]({
            message: <span style={{cursor: "pointer"}} onClick={() => openModelExcuse(item?.notification?.navigationId, item?.notification?.id)}>{item?.notification?.name}</span>,
            description: <span style={{cursor: "pointer"}} onClick={() => openModelExcuse(item?.notification?.navigationId, item?.notification?.id)}>{item?.notification?.description}</span>,
          });
        }
      })
